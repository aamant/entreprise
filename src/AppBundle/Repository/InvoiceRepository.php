...
class InvoiceRepository extends EntityRepository
{
    ...
    public function findForList(Company $company)
    {
        $query = $this->getEntityManager()
            ->createQuery('
                SELECT i, c, q, p FROM AppBundle:Invoice i
                JOIN i.customer c
    ...
    public function findWaitPaid(Company $company)
    {
        $query = $this->getEntityManager()
            ->createQuery("
                SELECT i, c, p FROM AppBundle:Invoice i
                JOIN i.customer c
                LEFT JOIN i.payments p
                WHERE i.company = :company
    ...
    public function getWaitToPaid(Company $company)
    {
        $query = $this->getEntityManager()
            ->createQuery("
                SELECT (SUM(i.total) - COALESCE(SUM(p.total), 0)) as total FROM AppBundle:Invoice i
                LEFT JOIN i.payments p
                WHERE i.company = :company
                AND i.status IN ('wait', 'partial')
            ")->setParameter('company', $company);
    ...
    public function findFullById($id)
    {
        $query = $this->getEntityManager()
            ->createQuery('
                SELECT p, c FROM AppBundle:Invoice p
                LEFT JOIN p.items c
                WHERE p.id = :id'
            )->setParameter('id', $id);
